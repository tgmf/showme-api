...
router.get('/lead/', (req, res) => {
    ...
    const query = `query { boards (ids:[${process.env.SHOWME_UNQUALIFIED_BOARD_ID}]) {items {name }}}`;
...
router.post('/', async (req, res) => {
    try {
        ...
        const result = await createItem(req.body, process.env.SHOWME_UNQUALIFIED_BOARD_ID, process.env.SHOWME_UNQUALIFIED_GROUP_NAME);
